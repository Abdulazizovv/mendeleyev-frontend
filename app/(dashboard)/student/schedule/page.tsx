...
export default function StudentSchedulePage() {
  ...
  const isToday         = useMemo(() => format(selectedDate, 'yyyy-MM-dd') === format(new Date(), 'yyyy-MM-dd'), [selectedDate]);
  ...
  // ── Time slots ─────────────────────────────────────────────────────────────
  const timeSlots = useMemo(() => (branchSettings ? computeTimeSlots(branchSettings) : []), [branchSettings]);
  ...
  if (classLoading) return <Skeleton className="h-[500px] w-full rounded-xl" />;
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-gray-200">
                ...
                <th className="px-3 py-2.5 text-left text-xs font-semibold text-green-700 bg-green-50/60">
                  {studentClass?.name} · Darslar
                </th>
              </tr>
            </thead>
            <tbody>
                    ...
                    <td className="p-2 align-top">
                      {lesson ? (
                        <div
                          className={cn(
                            ...
                            lesson.status === 'cancelled' || lesson.status === 'canceled'
                              ? 'border-l-red-300 bg-red-50/50 opacity-60'
                              : lesson.status === 'in_progress'
                              ? cn(subjectColor(lesson.subject_name ?? '').border, subjectColor(lesson.subject_name ?? '').bg, 'ring-2 ring-offset-1 ring-blue-200')
                              : cn(
                                  subjectColor(lesson.subject_name ?? '').border,
                                  subjectColor(lesson.subject_name ?? '').bg,
                        ...
                        <div className="h-10 rounded-lg border border-dashed border-gray-100 flex items-center justify-center">
                          <span className="text-xs text-gray-300">Bo'sh</span>
                        </div>
                      )}
                    </td>
                  </tr>
