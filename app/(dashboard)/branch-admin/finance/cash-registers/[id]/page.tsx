  ...
  if (!cashRegister) {
    return (
      <div className="flex items-center justify-center h-64 flex-col gap-3">
        ...
        <Button variant="outline" onClick={() => router.push("/branch-admin/finance")}>Orqaga</Button>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b bg-white sticky top-0 z-10">
        <Button variant="ghost" size="sm" onClick={() => router.push("/branch-admin/finance")}>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2">
            {loadingTx ? (
              ...
              transactions.map((tx) => (
                <div
                  key={tx.id}
                  onClick={() => router.push(`/branch-admin/finance/transactions/${tx.id}`)}
