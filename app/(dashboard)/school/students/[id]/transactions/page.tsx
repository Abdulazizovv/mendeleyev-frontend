...
export default function StudentTransactionsPage() {
  ...
  const { data: txData, isLoading: txLoading } = useQuery<PaginatedResponse<Transaction>>({
    queryKey: ["student-transactions", studentId, filters, searchTerm, page],
  ...
  const clearFilters = () => {
    setFilters({ transaction_type: "", payment_method: "", date_from: "", date_to: "", ordering: "-transaction_date" });
    setSearchTerm("");
    setPage(1);
  };
    ...
    <div className="max-w-5xl mx-auto space-y-4">
      ...
      <Tabs value={activeTab} onValueChange={(v) => { setActiveTab(v); clearFilters(); }}>
        <TabsList className="h-9">
          <TabsTrigger value="transactions" className="gap-1.5 text-sm" onClick={() => setPage(1)}>
            <Receipt className="w-3.5 h-3.5" />
            Tranzaksiyalar
          </TabsTrigger>
          <TabsTrigger value="payments" className="gap-1.5 text-sm" onClick={() => setPage(1)}>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-gray-100 bg-gray-50/60">
                          <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide px-5 py-3">Tur / Holat</th>
                          <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide px-3 py-3">Kategoriya</th>
                          <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide px-3 py-3">Oy uchun</th>
                          <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide px-3 py-3">To&apos;lov usuli</th>
                          <th className="text-left text-[11px] font-semibold text-gray-400 uppercase tracking-wide px-3 py-3">Sana</th>
                          <th className="text-right text-[11px] font-semibold text-gray-400 uppercase tracking-wide px-5 py-3">Summa</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-gray-50">
                        {txData.results.map((tx) => {
                          const isPos = ["income", "payment", "refund"].includes(tx.transaction_type);
                          const periodDisplay = tx.period_month
                            ? (() => {
                                const UZ_M = ["Yan","Feb","Mar","Apr","May","Iyn","Iyl","Avg","Sen","Okt","Noy","Dek"];
                                const [y, m] = tx.period_month.split("-");
                                return `${UZ_M[parseInt(m) - 1]} ${y}`;
                              })()
                            : null;
                          return (
                            <tr
                            ...
                            >
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  <div>
                                    ...
                                    <div className="mt-0.5">
                                      <span className={`text-[11px] font-medium px-1.5 py-0.5 rounded border ${TX_STATUS_COLOR[tx.status] ?? "bg-gray-50 text-gray-500 border-gray-200"}`}>
                                        {tx.status_display}
                                      </span>
                                    </div>
                                  </div>
                              ...
                              <td className={`px-5 py-3 text-sm font-bold tabular-nums text-right ${isPos ? "text-emerald-600" : "text-red-600"}`}>
                  ...
                  {txData.count > 20 && (
                    <div className="flex items-center justify-between px-5 py-4 border-t border-gray-100">
                      <p className="text-xs text-gray-400">
                        {(page - 1) * 20 + 1}–{Math.min(page * 20, txData.count)} / {txData.count} ta
                      </p>
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-xs font-semibold text-gray-600 px-2">
                          {page} / {Math.ceil(txData.count / 20)}
                        </span>
                        <Button
                          variant="outline"
                          size="sm"
                          className="h-8 px-3 text-xs"
                          disabled={page * 20 >= txData.count}
        ...
        <DialogContent className="max-w-sm">
          ...
          {selectedTx && (
            <div className="space-y-0 mt-1">
              ...
              <DetailRow label="Oy uchun" value={selectedTx.period_month ?? undefined} />
              <DetailRow label="Tavsif" value={selectedTx.description} />
              <DetailRow label="Referens" value={selectedTx.reference_number} />
              <DetailRow label="Sana" value={selectedTx.transaction_date ? formatDateTime(selectedTx.transaction_date) : formatDateTime(selectedTx.created_at)} />
            </div>
          )}
